            ...
            <section id="degerlendirmeler" class="bg-surface rounded-2xl p-6 shadow-2xs space-y-6">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <button type="button" @click="reviewFormOpen = !reviewFormOpen"
                            class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-ink hover:bg-terracotta text-white font-bold text-xs shadow-xs transition-colors cursor-pointer">
                ...
                @if($allReviews->isEmpty())
                    <p class="text-xs text-muted italic text-center py-4">Henüz yorum yapılmamış. İlk değerlendirmeyi siz bırakın!</p>
                @else
                    <div class="space-y-4">
                        @foreach($allReviews->take(6) as $rev)
                            <div class="p-4 rounded-xl bg-sand/60 space-y-2">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <div class="w-7 h-7 rounded-full bg-surface font-bold text-xs flex items-center justify-center text-ink shadow-2xs">
                                            {{ mb_substr($rev->author_name ?: 'M', 0, 1) }}
                                        </div>
                                        <div>
                                            <h4 class="font-bold text-xs text-ink">{{ $rev->author_name ?: 'Misafir' }}</h4>
                                            <span class="text-[10px] text-muted">{{ $rev->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                    <div class="flex items-center text-star">
                                        @for($i = 1; $i <= 5; $i++)
                                            <x-ico name="star" filled class="w-3.5 h-3.5 {{ $i <= $rev->rating ? 'text-star' : 'text-stone-300' }}" />
                                        @endfor
                                    </div>
                                </div>
                                @if($rev->comment)
                                    <p class="text-xs text-stone-700 leading-relaxed font-normal">{{ $rev->comment }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
