...
<header class="sticky top-0 z-40 bg-surface/95 backdrop-blur-md border-b border-warm shadow-xs">
        ...
        <div class="flex items-center justify-between h-18">
            ...
            <div class="flex items-center space-x-6">
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 group">
                    <div class="w-9 h-9 rounded-xl bg-terracotta text-white flex items-center justify-center font-black text-lg shadow-sm group-hover:bg-terracotta-dark">
                        M
                    </div>
                ...
                <div class="hidden lg:block relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false" type="button" class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-bold text-ink bg-sand hover:bg-surface rounded-lg border border-warm">
            ...
            <nav class="hidden md:flex items-center space-x-1 lg:space-x-2">
                <a href="{{ route('restaurants.index') }}" class="px-3.5 py-2 rounded-lg text-sm font-bold {{ request()->routeIs('restaurants.index') ? 'text-terracotta bg-orange-50/60' : 'text-ink hover:text-terracotta hover:bg-sand' }}">
                    Keşfet (Discover)
                </a>
                <a href="{{ route('map') }}" class="px-3.5 py-2 rounded-lg text-sm font-bold {{ request()->routeIs('map') ? 'text-terracotta bg-orange-50/60' : 'text-ink hover:text-terracotta hover:bg-sand' }}">
                    Harita (Map)
                </a>
                <a href="{{ route('categories') }}" class="px-3.5 py-2 rounded-lg text-sm font-bold {{ request()->routeIs('categories') ? 'text-terracotta bg-orange-50/60' : 'text-ink hover:text-terracotta hover:bg-sand' }}">
                    Kategoriler
                </a>
            </nav>
